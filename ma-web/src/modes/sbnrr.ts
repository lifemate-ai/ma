...
export interface SbnrrStepDef {
  key: string
  label: string
  duration: number // seconds
}

export const SBNRR_STEPS: SbnrrStepDef[] = [
...
export function mountSbnrr(container: HTMLElement, onDone: (sessionId?: string) => void) {
  let cancelled = false
  ...
  const runningArea = container.querySelector('#sbnrr-running') as HTMLElement
  const stepLabel = container.querySelector('#sbnrr-step-label') as HTMLElement
  const timerEl = container.querySelector('#sbnrr-timer') as HTMLElement
  const guideText = container.querySelector('#sbnrr-guide-text') as HTMLElement
