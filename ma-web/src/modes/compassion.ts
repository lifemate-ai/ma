...
export interface CompassionPhaseDef {
  key: string
  label: string
  phrases: string[]
}

export const COMPASSION_PHASES: CompassionPhaseDef[] = [
  {
    key: 'self',
    label: '自分へ',
    phrases: [
...
export function mountCompassion(container: HTMLElement, onDone: (sessionId?: string) => void) {
  let cancelled = false
  ...
  const runningArea = container.querySelector('#compassion-running') as HTMLElement
  const phaseLabel = container.querySelector('#compassion-phase-label') as HTMLElement
  const phraseText = container.querySelector('#compassion-phrase-text') as HTMLElement
