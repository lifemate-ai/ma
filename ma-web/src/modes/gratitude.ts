...
export function mountGratitude(container: HTMLElement, onDone: (sessionId?: string) => void) {
  let currentRound = 1
  ...
  const roundEl = container.querySelector('#gratitude-round') as HTMLElement
  const promptEl = container.querySelector('#gratitude-prompt') as HTMLElement
