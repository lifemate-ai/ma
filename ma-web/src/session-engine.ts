import type { CompanionMode, SessionMode } from './api'

export type InteractiveLegacyMode = 'emotion_mapping' | 'gratitude' | 'checkin'
export type TimedSessionMode = Exclude<SessionMode, InteractiveLegacyMode>
export type CueType =
  | 'arrival'
  | 'orient'
  | 'anchor'
...
export type VisualStyle = 'breath' | 'body' | 'still'
export type SafetyAction = 'stop' | 'shorter_close' | 'open_eyes' | 'grounding_return'
...
export interface SessionPlan {
  ...
  voiceDensity: 'sparse' | 'medium'
  visualStyle: VisualStyle
  breathCueDisplays?: [string, string]
  breathCueTts?: [string, string] | null
  breathCueIntervalSeconds?: number
  closeMode: CompanionMode
  cueSchedule: Cue[]
...
interface ProtocolSpec {
  ...
  breathCueTts?: [string, string] | null
  breathCueIntervalSeconds?: number
  buildCues: (durationSeconds: number) => Cue[]
}

const TIMED_MODES: readonly TimedSessionMode[] = [
  'yasashii',
  'motto_yasashii',
  'body_scan',
  'sbnrr',
...
export function isInteractiveLegacyMode(mode: SessionMode): mode is InteractiveLegacyMode {
  return mode === 'emotion_mapping' || mode === 'gratitude' || mode === 'checkin'
}

export function isTimedSessionMode(mode: SessionMode): mode is TimedSessionMode {
  return (TIMED_MODES as readonly string[]).includes(mode)
}

function pickDurationMinutes(requestedMinutes: number | undefined, durationsMinutes: number[]): number {
  if (!requestedMinutes) return durationsMinutes[0]
...
function clampAtSeconds(atSeconds: number, durationSeconds: number): number {
  return Math.max(0, Math.min(atSeconds, Math.max(0, durationSeconds - 8)))
}

function proportionalAt(durationSeconds: number, ratio: number): number {
...
function closeCue(durationSeconds: number, text: string): Cue {
  return cue('close', 'close', durationSeconds, durationSeconds - 14, text)
}

function bodyScanCues(durationSeconds: number): Cue[] {
  ...
  return segments.map(([id, ratio, text]) =>
    cue(String(id), id === 'whole' ? 'close' : 'orient', durationSeconds, proportionalAt(durationSeconds, Number(ratio)), String(text)),
  )
}

const PROTOCOLS: Record<TimedSessionMode, ProtocolSpec> = {
  yasashii: {
...
export function buildSessionPlan(mode: TimedSessionMode, requestedDurationMinutes?: number): SessionPlan {
...
export function nextCueIndex(plan: SessionPlan, elapsedSeconds: number): number {
  return plan.cueSchedule.findIndex(cue => cue.atSeconds > elapsedSeconds)
}

export function extendSessionPlan(plan: SessionPlan, elapsedSeconds: number): SessionPlan {
  ...
  if (nextMinutes) return buildSessionPlan(plan.sessionMode, nextMinutes)

  const fallbackMinutes = Math.max(
    currentMinutes + (plan.totalDurationSeconds >= 300 ? 2 : 1),
    Math.ceil((elapsedSeconds + 45) / 60),
  )
  return buildSessionPlan(plan.sessionMode, fallbackMinutes)
}

export function buildShorterCloseCue(plan: SessionPlan): Cue {
  return cue(
    'shorter-close',
    'safety',
    plan.totalDurationSeconds,
    0,
    '[reassuring][gently] ここで短く切り上げて大丈夫です。[pause] 足元か椅子との接地へ戻って、あと少しだけ静かに終えていきます。',
  )
}

export function buildGroundingReturnCue(plan: SessionPlan): Cue {
  return cue(
    'grounding-return',
    'safety',
    plan.totalDurationSeconds,
    0,
    '[reassuring][gently] いったん足元へ戻ります。[pause] 目を開けても大丈夫。[pause] 足の裏か、触れている面をひとつ選んで、そこへ戻ります。',
  )
}

export function buildOpenEyesCue(plan: SessionPlan): Cue {
  return cue(
    'open-eyes',
    'safety',
    plan.totalDurationSeconds,
    0,
    '[reassuring][softly] 目を開けて大丈夫です。[pause] 視界の中の形や明るさを見て、ここにいる感じを少し戻します。',
  )
}
