...
export function mountJournal(container, sessionId, onDone) {
    container.innerHTML = `
  ...
  `;
    ...
    const inputEl = container.querySelector('#journal-input');
    const loopArea = container.querySelector('#loop-area');
    const loopText = container.querySelector('#loop-text');
    ...
    doneBtn.addEventListener('click', async () => {
        if (userText && !saved) {
            saved = true;
            await persistJournal(false).catch(() => { });
        }
        onDone();
    });
    async function persistJournal(confirmed) {
        await saveJournal({
