...
import { greet, guide, closeSession, tts, saveSession, saveObservation, getCurriculumStatus } from './api';
...
export function mountSession(container, onDone, onHistory) {
    let state = null;
    let timer = null;
  ...
  `;
    ...
    // ── カリキュラム提案 ──────────────────────────────────────────
    const curriculumHintEl = container.querySelector('#curriculum-hint');
    getCurriculumStatus().then(status => {
        if (!status || status.suggested_modes.length === 0)
            return;
        const MODE_LABELS = {
    ...
    container.querySelector('#stop-btn').addEventListener('click', () => {
        ...
        if (timer)
            clearInterval(timer);
        const ra = container.querySelector('#running-area');
        if (ra?._cueTimer)
            clearInterval(ra._cueTimer);
        onDone(undefined);
    });
    // ── セッション開始 ───────────────────────────────────────────
    ...
    // ── 呼吸モード（やさしい / もっとやさしい） ──────────────────
    async function startBreathingSession(mode, guideEl, timerEl) {
        const modeHints = {
        ...
        const cueEl = container.querySelector('#breath-cue');
        ...
        const cueDecodedPromises = cueTtsTexts
        ...
        const switchCue = () => {
            ...
            setTimeout(async () => {
                ...
                if (!isGuideActive && !sessionEnded && cueTtsTexts) {
                    ...
                    if (decoded && !isGuideActive && !sessionEnded)
                        playDecoded(decoded);
                }
            }, 600);
        };
        const cueTimer = window.setInterval(switchCue, 4000);
        container.querySelector('#running-area')._cueTimer = cueTimer;
    ...
    // ── ボディスキャンモード ─────────────────────────────────────
    async function startBodyScan(guideEl, timerEl) {
        // 呼吸円を隠してボディスキャン用テキストエリアに
        const circleWrap = container.querySelector('#breath-circle-wrap');
    ...
    // ── 延長プロンプト ───────────────────────────────────────────
    async function showExtendPrompt(timerEl) {
    ...
    // ── タイマー表示更新 ─────────────────────────────────────────
    function updateTimerDisplay(el, elapsed) {
    ...
    async function endSession() {
        ...
        disableWatch();
        const runningArea = container.querySelector('#running-area');
        if (runningArea?._cueTimer)
            clearInterval(runningArea._cueTimer);
        playBell('end');
