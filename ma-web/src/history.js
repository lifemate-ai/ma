import { clearUserData, getUnifiedHistory } from './api';
...
export async function mountHistory(container, onBack, onEditPreferences) {
    container.innerHTML = `
  ...
  `;
    ...
    toolsEl.querySelector('#clear-all-data-btn').addEventListener('click', async () => {
        if (!window.confirm('この端末の記録を消しますか？'))
            return;
