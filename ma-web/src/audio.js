...
export function playBell(type = 'end') {
    stopCurrentAudio();
    const ac = getCtx();
...
export async function playAudioBuffer(buffer, isCancelled) {
    stopCurrentAudio();
    const ac = getCtx();
    const decoded = await ac.decodeAudioData(buffer);
    if (isCancelled?.())
        return; // decode中にキャンセルされた
 ...
 */
export async function playAudioStream(fetchFn) {
    stopCurrentAudio();
    ...
    return new Promise((resolve) => {
        ...
        const cleanup = () => {
            ...
            if (currentMediaAudio === audio)
                currentMediaAudio = null;
            if (currentMediaResolve && currentMediaAudio !== audio)
                currentMediaResolve = null;
        };
        ...
        ms.addEventListener('sourceopen', async () => {
            ...
            const appendChunk = (chunk) => new Promise(r => {
                sb.addEventListener('updateend', () => r(), { once: true });
                sb.appendBuffer(chunk.buffer.slice(chunk.byteOffset, chunk.byteOffset + chunk.byteLength));
            });
            try {
...
export function playDecoded(buf) {
    stopCurrentAudio();
    const ac = getCtx();
    const source = ac.createBufferSource();
    source.buffer = buf;
    ...
    currentSource = source;
    source.start();
    source.onended = () => { if (currentSource === source)
        currentSource = null; };
}
